...
class FeeService:
    ...
    def get_fee_rate(self) -> dict:
        if settings.use_stubs:
            return {"fee_rate": self._stub_fee_rate, "message": "Current exchange fee rate."}
        with httpx.Client() as client:
            response = client.get(
                f"{settings.exchange_base_url}/admin/market/status",
                headers=self._admin_headers(),
            )
            data = self._handle_response(response)
            return {"fee_rate": data.get("fee_rate", 0.001), "message": "Current exchange fee rate."}

    def update_fee_rate(self, req: FeeRateUpdateRequest) -> dict:
        if settings.use_stubs:
            self._stub_fee_rate = req.rate
            return {"fee_rate": self._stub_fee_rate, "message": f"Fee rate updated to {req.rate}."}
        with httpx.Client() as client:
            response = client.put(
            ...
            data = self._handle_response(response)
            return {"fee_rate": data.get("rate", req.rate), "message": f"Fee rate updated to {req.rate}."}
